...
# Doctor Recent Chat Conversations Endpoint
@app.route('/api/chat/doctor/<int:doctor_id>/recent', methods=['GET'])
def get_doctor_recent_chat_conversations(doctor_id):
    try:
        # Get recent messages for the doctor (simpler approach)
        messages = db.session.query(Message).filter(
            or_(
                and_(Message.sender_id == doctor_id, Message.sender_type == 'doctor'),
                and_(Message.receiver_id == doctor_id, Message.receiver_type == 'doctor')
        ...
        for msg in messages:
            # Determine the patient involved
            if msg.sender_type == 'doctor' and msg.sender_id == doctor_id:
                # Doctor sent message to patient
                patient_id = msg.receiver_id
                is_from_doctor = True
            elif msg.receiver_type == 'doctor' and msg.receiver_id == doctor_id:
            ...
            # Skip if we already have a conversation with this patient
            if patient_id in seen_patients:
                continue
            seen_patients.add(patient_id)
            
            # Get patient info
            patient = db.session.query(User).filter(User.patient_id == patient_id).first()
            
            if patient:
                conversation_data = {
                    'id': msg.id,
                    'content': msg.content or 'Image message',
                    'timestamp': msg.timestamp.isoformat() if msg.timestamp else None,
                    'is_read': msg.is_read,
                    'is_from_doctor': is_from_doctor,
                    'sender': {
